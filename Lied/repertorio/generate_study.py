from music21 import stream, scale, note, tempo, meter, key, chord, roman, converter
import random
import base64
...
class GeneradorEjercicios:
    ...
    def generar_ejercicio(self, tipo):
        ...
        # === SOLUCIÓN CORREGIDA: Usar converter.toData para obtener XML sin archivos temporales ===
        # Obtener el XML como string directamente desde music21
        xml_string = converter.toData(score, 'musicxml')
        
        # Convertir string a bytes para guardar
        contenido_bytes = xml_string.encode('utf-8')
        ...
        ejercicio = Ejercicio(
            ...
            contenido=base64.b64encode(contenido_bytes).decode('utf-8')
        )
        
        # Guardar el archivo MusicXML directamente desde los bytes
        ejercicio.archivo_musicxml.save(
            f"{tipo}_{timezone.now().timestamp()}.musicxml",
